...
// Nachschlage-Übersicht aller Haltbarkeits-Richtwerte (nach dem Öffnen).
export function ShelfLifeSheet({ open, onClose, t }) {
  ...
  const list = useMemo(() => {
    ...
    if (!query) return OPENED_SHELF_RULES;
    return OPENED_SHELF_RULES.filter(
      (r) => r.label.toLowerCase().includes(query) || r.keys.some((k) => k.includes(query)),
    );
  }, [q]);

  return (
    <Modal open={open} onClose={onClose} t={t} title="Haltbarkeits-Ratgeber" subtitle="Richtwerte nach dem Öffnen">
