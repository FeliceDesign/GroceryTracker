...
import { hasMacros, macroSummary, basisLabel } from '../../lib/macros.js';

// Aufklappbarer „Nährwerte"-Abschnitt für das Bearbeiten-/Anlegen-Sheet.
// Zeigt zugeklappt eine Kurz-Zusammenfassung, aufgeklappt den vollen Editor.
export function MacroSection({ name, macros, onChange, t, scanSupported, accent }) {
  const filled = hasMacros(macros);
  const [open, setOpen] = useState(filled);
  const summary = filled ? `${macroSummary({ ...macros })} · ${basisLabel(macros)}` : 'noch keine – tippen zum Erfassen';
    ...
    <div style={{ marginTop: 18, border: `1px solid ${t.border}`, borderRadius: 14, overflow: 'hidden' }}>
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          <span style={{ display: 'block', fontSize: 14, fontWeight: 700, color: t.text }}>Nährwerte</span>
          <span style={{
