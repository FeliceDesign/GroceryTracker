...
export function parseNutritionFacts(rawText) {
  ...
  for (const line of lines) {
    ...
    if (key === 'kcal') {
      assign('kcal', extractEnergy(low));
      continue;
    }

    // Bezugsangabe entfernen, damit nicht die „100" als Wert gelesen wird.
    const num = extractNumber(low.replace(/100\s*(g|ml)/g, ' '));
    if (!num) continue;
    ...
    if (key === 'salt' && /natrium|sodium/.test(low) && !/salz|salt/.test(low)) {
      value *= 2.5;
    }
