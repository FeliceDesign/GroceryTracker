...
export async function scanBarcode() {
  ...
  try {
    ...
    // isGoogleBarcodeScannerModuleAvailable existiert nur auf Android –
    // andere Plattformen überspringen den Schritt und scannen direkt.
  }
...
// ---------------------------------------------------------------------------
// Open Food Facts – Barcode zu Produktdaten
// ---------------------------------------------------------------------------
...
function parseQuantity(quantityStr) {
  ...
  if (!Number.isFinite(num) || num <= 0) return { qty: 1, unit: 'stk' };
  switch (m[2]) {
...
function normalizeYear(yy) {
  ...
  return 2000 + n; // zweistellig: bei Lebensmitteln immer 20xx
}
...
export function parseBestBeforeDate(text) {
  ...
  // MHD ist in der Regel das späteste plausible Datum.
  pool.sort((a, b) => b - a);
  return toISO(pool[0]);
}
...
export async function capturePhotoAndReadDate() {
  ...
  // takePhoto liefert uri teils als reinen Dateipfad ohne Schema.
